...
pub enum StepOutcome {
    ...
    /// The compiled workflow needs recompilation (e.g. variant mismatch).
    /// The worker should create a recompile plan-mode session immediately
    /// instead of falling through to the generic failure path.
    NeedsRecompile {
        reason: String,
        failure_kind: String,
    },
    ...
    pub async fn run_step(
        ...
        if plan.is_none() {
            ...
            let new_plan = if is_direct_response_goal(&state.goal) {
                ...
                workflow_plan
            } else if state.metadata.get("needs_recompile").and_then(|v| v.as_bool()).unwrap_or(false) {
                // Variant mismatch or other compiler-detected issue — signal
                // the worker to create a recompile plan-mode session directly
                // instead of falling through to the generic failure path.
                let reason = state
                    .metadata
                    .get("recompile_reason")
                    .and_then(|v| v.as_str())
                    .unwrap_or("compiled workflow needs recompilation")
                    .to_string();
                let failure_kind = state
                    .metadata
                    .get("recompile_failure_kind")
                    .and_then(|v| v.as_str())
                ...
                tracing::warn!(
                    ...
                    "compiled workflow flagged for recompilation"
                );
                return Ok(StepOutcome::NeedsRecompile { reason, failure_kind });
            } else {
                // No compiled workflow and no recompile flag — hard fail.
                let reason = format!(

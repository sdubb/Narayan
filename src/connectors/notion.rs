...
pub struct NotionConnector { http: reqwest::Client }
...
impl Connector for NotionConnector {
    fn connector_type(&self) -> &str { "notion" }
    ...
    async fn validate_config(&self, config: &ConnectorConfig) -> Result<()> {
        ...
        let resp  = self.http.get("https://api.notion.com/v1/users/me")
            .bearer_auth(&token)
            .header("Notion-Version", "2022-06-28")
            .send().await?;
        if !resp.status().is_success() { anyhow::bail!("Notion auth failed: {}", resp.status()); }
        Ok(())
    }
}

impl Default for NotionConnector { fn default() -> Self { Self::new() } }

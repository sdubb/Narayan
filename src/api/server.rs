...
pub fn build_router(
    ...
    let auth_state   = AuthState { tenant_store: tenant_store.clone(), jwt_secret: state.jwt_secret.clone() };
    ...
    let register_governor = Arc::new(
        GovernorConfigBuilder::default()
            .per_second(12)  // 5 per minute = 1 per 12s
            .burst_size(5)
            .finish()
    ...
    let public = Router::new()
        ...
        .route("/auth/register", post(register).layer(GovernorLayer { config: register_governor }))

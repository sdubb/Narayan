...
impl StripeProvider {
    pub fn new(secret_key: String, webhook_secret: String) -> Self {
        Self {
            secret_key,
            webhook_secret,
            http: Client::builder().timeout(std::time::Duration::from_secs(30)).build().expect("reqwest client"),
        }
    }
    ...
    fn verify_stripe_signature(payload: &[u8], signature: &str, secret: &str) -> Result<()> {
        ...
        let provided_sigs: Vec<&str> =
            signature.split(',').filter(|p| p.starts_with("v1=")).filter_map(|p| p.strip_prefix("v1=")).collect();
        ...
        let event = match event_type.as_str() {
            ...
            "invoice.payment_failed" => {
                ...
                BillingEvent::PaymentFailed {
                    provider_subscription_id: sub_id,
                    tenant_id,
                    reason: data["last_finalization_error"]["message"].as_str().unwrap_or("payment failed").to_string(),
                }
            }

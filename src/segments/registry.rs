...
impl SegmentServices {
    pub fn none() -> Self {
        Self { policy: None, citations: None, sla: None, reviews: None, evidence: None, pii: None }
    }
}
...
impl SegmentRegistryBuilder {
    ...
    pub fn build(self) -> SegmentRegistry {
        ...
        for plugin in self.plugins {
            ...
            if let Some(p) = plugin.services.policy {
                merged_services.policy = Some(p);
            }
            if let Some(c) = plugin.services.citations {
                merged_services.citations = Some(c);
            }
            if let Some(r) = plugin.services.reviews {
                merged_services.reviews = Some(r);
            }
            if let Some(e) = plugin.services.evidence {
                merged_services.evidence = Some(e);
            }
            if let Some(p) = plugin.services.pii {
                merged_services.pii = Some(p);
            }
        ...
        let sla_tracker =
            if all_sla_policies.is_empty() { None } else { Some(Arc::new(SlaTracker::new(all_sla_policies))) };

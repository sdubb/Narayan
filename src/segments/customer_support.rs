...
pub fn plugin(deps: &SharedDeps, tenant_id: &str) -> SegmentPlugin {
    ...
    rules.rules.push(PolicyRule {
        ...
        action: PolicyAction::RequireApproval {
            message: "Refund over $100 requires human approval".into(),
        },
        enabled: true,
    });
    ...
    rules.rules.push(PolicyRule {
        ...
        action: PolicyAction::Redact {
            fields: vec!["ssn".into(), "credit_card".into(), "password".into()],
        },
        enabled: true,
    });
        ...
        sla_policies: vec![
            SlaPolicy {
                ...
                escalation_rules: vec![
                    EscalationRule { trigger_pct: 80.0, action: EscalationAction::Notify { message: "Urgent SLA at 80%".into() } },
                    EscalationRule { trigger_pct: 100.0, action: EscalationAction::EscalateToHuman { reason: "Urgent SLA breached".into() } },
                ],
            },
            SlaPolicy {
                ...
                escalation_rules: vec![
                    EscalationRule { trigger_pct: 80.0, action: EscalationAction::Notify { message: "Normal SLA at 80%".into() } },
                ],
            },
        ],
    }
